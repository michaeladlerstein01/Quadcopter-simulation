    ...
    methods
        ...
        function obj = Drone(axis, spaceDim, num_drones)
            if nargin > 1
                ...
                obj.omega = [0;0;0];
                obj.anglesdot = [0;0;0];
                obj.init_state = [0;0;5;0;0;0;0;0;0;0;0;0];
                obj.xd = zeros(12,1);
                obj.pre_i = obj.i ;
                obj.delta_x = zeros(12 ,1);
                obj.x  = obj.init_state;
                obj.A_d = eye(12);
                obj.B_d = eye(12 ,4);
                obj.input= obj.m*obj.g/4;
...
%         end
        ...
        function update(obj)
            ...
            if obj.time == 0.02
                [obj.A_d , obj.B_d] = linearisation(obj) ;  
                obj.delta_x = zeros(12, 1);
                obj.x = obj.init_state;
                obj.pre_i = obj.input;
            ...
            if obj.time > 2 && obj.time < 4
                obj.i(:) = 1.15*obj.input;                
            end 
            if obj.time > 4 && obj.time <= 8   
                obj.i(3)=0;
            end 
            obj.delta_x = obj.A_d * obj.delta_x + obj.B_d * (obj.i - obj.pre_i);
            obj.x = obj.x + obj.delta_x;
            obj.pre_i = obj.i;

            obj.pos = obj.x(1:3);
            obj.xdot = obj.x(4:6);
            obj.angles = obj.x(7:9);
            obj.omega = obj.x(10:12);
            obj.R = eul2rotm(obj.angles');
